...
namespace Coolify.Net.Tests.Acceptance.Databases
{
    public partial class DatabaseClientTests
    {
        ...
        public async Task ShouldStartDatabaseAsync()
        {
            ...
            // when
            await this.clientBroker.Client.Databases.StartDatabaseAsync(databaseUuid);

            // then
            this.clientBroker.Server
                .FindLogEntries(Request.Create().WithPath($"/api/v1/databases/{databaseUuid}/start").UsingPost())
