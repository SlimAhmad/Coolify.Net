...
namespace Coolify.Net.Tests.Acceptance.Applications
{
    public partial class ApplicationClientTests
    {
        ...
        public async Task ShouldRemoveApplicationAsync()
        {
            ...
            // when
            await this.clientBroker.Client.Applications.RemoveApplicationAsync(applicationUuid);

            // then
            this.clientBroker.Server
                .FindLogEntries(Request.Create().WithPath($"/api/v1/applications/{applicationUuid}").UsingDelete())
