...
namespace Coolify.Net.Tests.Unit.Services.Foundations.Databases
{
    public partial class DatabaseServiceTests
    {
        ...
        public async Task ShouldThrowDependencyExceptionOnStopDatabaseIfHttpErrorOccursAndLogItAsync(
            HttpStatusCode statusCode)
        {
            ...
            this.coolifyApiBrokerMock
                .Setup(broker => broker.PostDatabaseStopAsync(someDatabaseUuid))
                .ThrowsAsync(httpRequestException);

            // when
            ValueTask stopDatabaseTask =
                this.databaseService.StopDatabaseAsync(someDatabaseUuid);

            DatabaseDependencyException actualException =
                await Assert.ThrowsAsync<DatabaseDependencyException>(stopDatabaseTask.AsTask);

            // then
            actualException.Should().BeEquivalentTo(expectedException);

            this.coolifyApiBrokerMock.Verify(broker =>
                broker.PostDatabaseStopAsync(someDatabaseUuid), Times.Once);
        ...
        [Fact]
        public async Task ShouldThrowServiceExceptionOnStopDatabaseIfServiceErrorOccursAndLogItAsync()
        {
            // given
            string someDatabaseUuid = GetRandomString();
            ...
            DatabaseServiceException actualException =
                await Assert.ThrowsAsync<DatabaseServiceException>(stopDatabaseTask.AsTask);
