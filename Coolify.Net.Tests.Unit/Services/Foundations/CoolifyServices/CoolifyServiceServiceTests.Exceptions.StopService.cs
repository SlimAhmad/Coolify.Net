...
namespace Coolify.Net.Tests.Unit.Services.Foundations.CoolifyServices
{
    public partial class CoolifyServiceServiceTests
    {
        ...
        public async Task ShouldThrowDependencyExceptionOnStopServiceIfHttpErrorOccursAndLogItAsync(
            HttpStatusCode statusCode)
        {
            ...
            this.coolifyApiBrokerMock
                .Setup(broker => broker.PostServiceStopAsync(someServiceUuid))
                .ThrowsAsync(httpRequestException);

            // when
            ValueTask stopServiceTask =
                this.coolifyServiceService.StopServiceAsync(someServiceUuid);

            CoolifyServiceDependencyException actualException =
                await Assert.ThrowsAsync<CoolifyServiceDependencyException>(stopServiceTask.AsTask);

            // then
            actualException.Should().BeEquivalentTo(expectedException);

            this.coolifyApiBrokerMock.Verify(broker =>
                broker.PostServiceStopAsync(someServiceUuid), Times.Once);
        ...
        [Fact]
        public async Task ShouldThrowServiceExceptionOnStopServiceIfServiceErrorOccursAndLogItAsync()
        {
            // given
            string someServiceUuid = GetRandomString();
            ...
            CoolifyServiceServiceException actualException =
                await Assert.ThrowsAsync<CoolifyServiceServiceException>(stopServiceTask.AsTask);
