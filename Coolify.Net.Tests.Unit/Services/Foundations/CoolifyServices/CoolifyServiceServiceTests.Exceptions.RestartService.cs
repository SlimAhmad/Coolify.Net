...
namespace Coolify.Net.Tests.Unit.Services.Foundations.CoolifyServices
{
    public partial class CoolifyServiceServiceTests
    {
        ...
        public async Task ShouldThrowDependencyExceptionOnRestartServiceIfHttpErrorOccursAndLogItAsync(
            HttpStatusCode statusCode)
        {
            ...
            this.coolifyApiBrokerMock
                .Setup(broker => broker.PostServiceRestartAsync(someServiceUuid))
                .ThrowsAsync(httpRequestException);

            // when
            ValueTask restartServiceTask =
                this.coolifyServiceService.RestartServiceAsync(someServiceUuid);

            CoolifyServiceDependencyException actualException =
                await Assert.ThrowsAsync<CoolifyServiceDependencyException>(restartServiceTask.AsTask);

            // then
            actualException.Should().BeEquivalentTo(expectedException);

            this.coolifyApiBrokerMock.Verify(broker =>
                broker.PostServiceRestartAsync(someServiceUuid), Times.Once);
        ...
        [Fact]
        public async Task ShouldThrowServiceExceptionOnRestartServiceIfServiceErrorOccursAndLogItAsync()
        {
            // given
            string someServiceUuid = GetRandomString();
            ...
            CoolifyServiceServiceException actualException =
                await Assert.ThrowsAsync<CoolifyServiceServiceException>(restartServiceTask.AsTask);
